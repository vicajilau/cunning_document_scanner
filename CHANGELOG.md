...
### Changed
* **Corrected what the README says about where scans live and how long they last.** It described a single "plugin-owned cache directory" on both platforms, claimed nothing was backed up and that the system would reclaim the files. That holds for iOS, whose output sits in `Library/Caches/`, but not for Android: every scan is written to the application's private external pictures directory, which is not a cache. Android never clears it, and its contents are included in Auto Backup unless the application opts out. Applications relying on the documented behavior were leaving scans on disk indefinitely.
